...
class ImageUploadHorizontalListState<T extends ImageUploadHorizontalList>
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Padding(
              padding: const EdgeInsets.only(right: 16.0),
              child: Text(
                'Вы можете загрузить до ${widget.maxImageCount} фотографий.'.tr,
  ...
  Future<void> chooseVideo() async {
    ...
    try {
      ...
      print('Недопустимая операция' + e.toString());
    } catch (ex) {
      print(ex);
    }
      ...
      if (videoDuration < maximumSecond) {
        ...
        showDialog<dynamic>(
            ...
            builder: (BuildContext context) {
              return ErrorDialog(
                  message:
                      'К сожалению, продолжительность видео не должна превышать ${maximumSecond / 1000} секунд.');
            });
      }
    }
  }
