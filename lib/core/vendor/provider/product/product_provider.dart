...
class ItemDetailProvider extends PsProvider<Product> {
  ...
  void addIntentAddOnList(
    ...
    if (intentBasketSelectedAddOnList != null) {
      for (BasketSelectedAddOn obj in intentBasketSelectedAddOnList) {
        selectedAddOnMapList[AddOn(
            id: obj.id,
            name: obj.name,
            price: obj.price)] = true;
  ...
  void addIntentCustomizedList(
    ...
    if (intentBasketSelectedAttributeList != null) {
      for (BasketSelectedAttribute obj
          in intentBasketSelectedAttributeList) {
        selectedcustomizedDetailMapList[obj.headerId!] =
            CustomizedDetail(
                id: obj.id,
  ...
  void addCustomizedDetail(CustomizedDetail customizedDetail) {
    basketSelectedAttribute.addAttribute(
      basketSelectedAttribute.fromCustomizedDetail(customizedDetail, product.itemCurrency?.currencySymbol));
    selectedCustomizedPrice += double.tryParse(customizedDetail.additionalPrice ?? '') ?? 0.0;
    notifyListeners();
  }
  ...
  Future<PsResource<ApiStatus>> userDeleteItem(
    ...
    String loginUserId, String languageCode, String itemId
  ) async {
    isLoading = true;
    ...
    return await _repo!.userDeleteItem(
        jsonMap, loginUserId, itemId, psValueHolder!.headerToken!, languageCode, isConnectedToInternet, PsStatus.PROGRESS_LOADING);
  }
}
