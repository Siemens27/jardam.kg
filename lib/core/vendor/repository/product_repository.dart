...
class ProductRepository extends PsRepository {
    ...
    await startResourceSinkingForOne(
      ...
      serverRequestCallback: () async {
        return _psApiService.getItemDetail(
            requestPathHolder?.itemId, requestPathHolder?.loginUserId, requestPathHolder?.languageCode ?? 'en');
      },
    );
  }
  ...
  Future<dynamic> getItemDetailForFav(
    ...
    if (isConnectedToInternet) {
      final PsResource<Product> _resource =
          await _psApiService.getItemDetail(itemId, loginUserId, languageCode ?? 'en');
  ...
  Future<dynamic> getAllFavouritesList(
    ...
    await startResourceSinkingForListWithJoin<FavouriteProduct>(
      ...
      serverRequestCallback: () =>
          _psApiService.getFavouritesList(loginUserId, headerToken!, languageCode ?? 'en', limit, offset),
    );
  ...
  Future<PsResource<Product>> postFavourite(Map<dynamic, dynamic> jsonMap,
      ...
      {bool isLoadFromServer = true}) async {
    final PsResource<Product> _resource =
        await _psApiService.postFavourite(jsonMap, loginUserId, headerToken, languageCode ?? 'en');
    if (_resource.status == PsStatus.SUCCESS) {
  ...
  Future<PsResource<ApiStatus>> postData({
    ...
    final PsResource<ApiStatus> _resource = await _psApiService.postTouchCount(
        requestBodyHolder!.toMap(), requestPathHolder?.loginUserId ?? '', requestPathHolder?.languageCode ?? 'en');
    if (_resource.status == PsStatus.SUCCESS) {
      return _resource;
    } else {
      final Completer<PsResource<ApiStatus>> completer =
  ...
  Future<dynamic> getRelatedProductList(
    ...
    await startResourceSinkingForListWithJoin<RelatedProduct>(
      ...
      serverRequestCallback: () => _psApiService.getRelatedProductList(
          productId, categoryId, loginUserId, languageCode ?? 'en', limit, offset),
    );
  }
  ...
  Future<dynamic> markSoldOutItem(
    ...
    startResourceSinkingForOne(
        ...
        dataConfig: dataConfig,
        serverRequestCallback: () =>
            _psApiService.markSoldOutItem(holder!.toMap(), loginUserId, headerToken, languageCode ?? 'en'));
  ...
  Future<PsResource<Product>> postItemEntry(
      Map<dynamic, dynamic> jsonMap, String loginUserId, String headerToken, String? languageCode, PsStatus status,
      {bool isLoadFromServer = true}) async {
    final PsResource<Product> _resource =
        await _psApiService.postItemEntry(jsonMap, loginUserId, headerToken, languageCode ?? 'en');
  ...
  Future<PsResource<ApiStatus>> userDeleteItem(Map<dynamic, dynamic> jsonMap, 
      String loginUserId, String itemId, String headerToken, String? languageCode,
      bool isConnectedToInternet, PsStatus status,
      {bool isLoadFromServer = true}) async {
    final PsResource<ApiStatus> _resource =
        await _psApiService.deleteItem(jsonMap, loginUserId, itemId, headerToken, languageCode ?? 'en');
    if (_resource.status == PsStatus.SUCCESS) {
      return _resource;
    } else {
      final Completer<PsResource<ApiStatus>> completer =
